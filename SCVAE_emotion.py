...
from model.model_word2vec_emotion import Model
...
parser.add_argument('--trainset_path', dest='trainset_path', default='data/raw/emotion/emotion_trainset_classify.txt', type=str, help='训练集位置')
parser.add_argument('--validset_path', dest='validset_path', default='data/raw/emotion/emotion_validset_classify.txt', type=str, help='验证集位置')
parser.add_argument('--testset_path', dest='testset_path', default='data/raw/emotion/emotion_testset_classify.txt', type=str, help='测试集位置')
parser.add_argument('--embed_path', dest='embed_path', default='data/raw/vocab.txt', type=str, help='词向量位置')
parser.add_argument('--result_path', dest='result_path', default='log_emotion', type=str, help='测试结果位置')
...
parser.add_argument('--log_path', dest='log_path', default='log_emotion', type=str, help='记录模型位置')
...
parser.add_argument('--model_path', dest='model_path', default='log_emotion/emotion1637321073/negative_120000000490500.model', type=str, help='载入模型位置')  #
parser.add_argument('--seed', dest='seed', default=777, type=int, help='随机种子')  #
...
def main():
    ...
    negative_train, negative_valid, negative_test = [], [], []
    positive_train, positive_valid, positive_test = [], [], []
    no_emotion_train, no_emotion_valid, no_emotion_test = [], [], []
    emotion_trainset, emotion_validset, emotion_testset = [], [], []
    if args.inference:  # 测试时只载入测试集
        ...
        with open('data/raw/emotion/negative_train.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                negative_train.append(json.loads(line))
        print(f'negative_train:{len(negative_train)}条')
        with open('data/raw/emotion/negative_valid.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                negative_valid.append(json.loads(line))
        print(f'negative_valid:{len(negative_valid)}条')
        with open('data/raw/emotion/negative_test.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                negative_test.append(json.loads(line))
        print(f'negative_test:{len(negative_test)}条')
        with open('data/raw/emotion/positive_train.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                positive_train.append(json.loads(line))
        print(f'positive_train:{len(positive_train)}条')
        with open('data/raw/emotion/positive_valid.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                positive_valid.append(json.loads(line))
        print(f'positive_valid:{len(positive_valid)}条')
        with open('data/raw/emotion/positive_test.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                positive_test.append(json.loads(line))
        print(f'positive_test:{len(positive_test)}条')
        with open('data/raw/emotion/no_emotion_train.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                no_emotion_train.append(json.loads(line))
        print(f'no_emotion_train:{len(no_emotion_train)}条')
        with open('data/raw/emotion/no_emotion_valid.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                no_emotion_valid.append(json.loads(line))
        print(f'no_emotion_valid{len(no_emotion_valid)}条')
        with open('data/raw/emotion/no_emotion_test.txt', 'r', encoding='utf8') as fr:
            for line in fr:
                no_emotion_test.append(json.loads(line))
        print(f'no_emotion_test:{len(no_emotion_test)}条')

    emotion_trainset.append(no_emotion_train)
    emotion_trainset.append(positive_train)
    emotion_trainset.append(negative_train)
    emotion_validset.append(no_emotion_valid)
    emotion_validset.append(positive_valid)
    emotion_validset.append(negative_valid)
    emotion_testset.append(no_emotion_test)
    emotion_testset.append(positive_test)
    emotion_testset.append(negative_test)
    cat_num = ['no_emotion', 'positive', 'negative']
    # 载入词汇表，词向量
    vocab_d ={}
    vocab = []
    ...
    if os.path.isfile(args.model_path):  # 如果载入模型的位置存在则载入模型
        ...
        print('初始化模型完成')
        # 记录模型的文件夹
        log_dir = os.path.join(args.log_path, 'emotion' + str(int(time.time())))
    ...
    if not args.inference:# 训练
        for i in range(0, len(emotion_trainset)):
            epoch = 0
            args.ingau = i
            summary_writer = SummaryWriter(os.path.join(log_dir, 'summary'))  # 创建tensorboard记录的文件夹
            dp_train = DataProcessor(emotion_trainset[i], config.batch_size, sentence_processor)  # 数据的迭代器
            dp_valid = DataProcessor(emotion_validset[i], config.batch_size, sentence_processor, shuffle=False)
        ...
        result_file = os.path.join(args.result_path, 'a-emotion_cvae_6epoch.txt')  # 命名结果文件
        ...
        #     outputs_0, outputs_1, outputs_2, outputs, clf_result = test(model, feed_data)  # 使用模型计算结果 [batch, len_decoder]
        #     for idx in range(0, len(outputs_0)):
        #         new_data = dict()
        #         new_data['post'] = posts[idx]
        #         new_data['response'] = responses[idx]
        #         new_data['no_emotion'] = sentence_processor.index2word(outputs_0[idx])  # 将输出的句子转回单词的形式
        #         new_data['positive'] = sentence_processor.index2word(outputs_1[idx])  # 将输出的句子转回单词的形式
        #         new_data['negative'] = sentence_processor.index2word(outputs_2[idx])  # 将输出的句子转回单词的形式
        #         new_data['no_label'] = sentence_processor.index2word(outputs[idx])  # 将输出的句子转回单词的形式
        #         new_data['clf_result'] = clf_result[idx]
        #         len_results.append(len(new_data['no_emotion'])+len(new_data['positive'])+len(new_data['negative']))
        #         len_no_label.append(len(new_data['no_label']))
        #         fw.write(json.dumps(new_data, ensure_ascii=False) + '\n')
        #
        # fw.close()
        # print(f'生成句子平均长度: {1.0 * sum(len_results) / (len(len_results)*3)}')
...
def train(model, feed_data, global_step):
    ...
    labels_clf = feed_data['post_emotion']
...
def valid(model, data_processor, global_step):
    ...
    for data in data_processor.get_batch_data():
        feed_data = prepare_feed_data(data)
        output_vocab, _mu, _logvar, mu, logvar, output_classify = model(feed_data, word2vec, inpre=args.inpre, ingau=2, gpu=args.gpu)
...
def test(model, feed_data):
    outputs_0, outputs_1, outputs_2,outputs, clf_result = model(feed_data, word2vec, inference=True, max_len=args.max_len, gpu=args.gpu)
