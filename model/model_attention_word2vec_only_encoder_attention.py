...
from model.Only_Encoder_attention import Encoder
...
class Model(nn.Module):
    ...
    def forward(self, inputs,word2vec, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if inpre:
            ...
            _, state_posts = self.post_encoder(embed_posts.transpose(0, 1), len_encoder=len_posts)
            _, state_responses = self.response_encoder(embed_responses.transpose(0, 1), len_encoder=len_responses)
            x = state_posts[-1][1]  # [batch, dim]
            y = state_responses[-1][1]  # [batch, dim]
            ...
            embed_posts = word2vec.embedding(id_posts).transpose(0, 1)  # [batch, seq, embed_size]
            embed_posts = embed_posts.split([1] * id_posts.size(1), 0)
            embed_responses = word2vec.embedding(id_responses).transpose(0, 1)  # [batch, seq, embed_size]
            embed_responses = embed_responses.split([1] * id_responses.size(1), 0)
            # state: [layers, batch, dim]
            post_encoder_output=torch.tensor([]).cuda()
            response_encoder_output=torch.tensor([]).cuda()

            for idx in range(id_posts.size(1)):
                if idx == 0:
                    h_0 = Variable(torch.zeros(self.config.post_encoder_num_layers, self.config.batch_size,
                                               self.config.post_encoder_output_size)).cuda()
                    c_0 = Variable(torch.zeros(self.config.post_encoder_num_layers, self.config.batch_size,
                                               self.config.post_encoder_output_size)).cuda()
                    state_posts = (h_0, c_0)
                post_encoder_output, state_posts, post_encoder_output= self.post_encoder(embed_posts[idx], state=state_posts, encoder_outputs=post_encoder_output)

            for idx in range(id_responses.size(1)):
                if idx == 0:
                    h_0 = Variable(torch.zeros(self.config.post_encoder_num_layers, self.config.batch_size,
                                               self.config.post_encoder_output_size)).cuda()
                    c_0 = Variable(torch.zeros(self.config.post_encoder_num_layers, self.config.batch_size,
                                               self.config.post_encoder_output_size)).cuda()
                    state_responses = (h_0, c_0)
                response_encoder_output, state_responses, response_encoder_output = self.response_encoder(embed_responses[idx], state=state_responses, encoder_outputs=response_encoder_output)
            ...
            # 解码器的输入为回复去掉end_id
            decoder_inputs = word2vec.embedding(id_responses)[:, :-1, :].transpose(0, 1)  # [seq-1, batch, embed_size]
            ...
            for idx in range(id_posts.size(1)):
                if idx == 0:
                    ...
                    c_0 = Variable(torch.zeros(self.config.post_encoder_num_layers, self.config.batch_size,
                                               self.config.post_encoder_output_size)).cuda()
                    state_posts = (h_0, c_0)
                post_encoder_output, state_posts, post_encoder_output = self.post_encoder(embed_posts[idx],
                                                                                          state=state_posts,
                                                                                          encoder_outputs=post_encoder_output)
