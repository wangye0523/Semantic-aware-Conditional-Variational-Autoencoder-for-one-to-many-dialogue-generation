...
class Model(nn.Module):
    ...
    def forward(self, inputs,word2vec,mu1,sigma1, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if inpre:
            ...
            # p(z|x)
            _mu, _logvar = self.prior_net(x)  # [batch, latent]
            _mu = mu1[int(str_responses_act[0][0])-1]
            _logvar = sigma1[int(str_responses_act[0][0]) - 1]
            # 重参数化
