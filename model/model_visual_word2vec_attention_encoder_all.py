    ...
    def forward(self, inputs, word2vec, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        ...
        with torch.no_grad():
            for data in inputs.get_batch_data():
                ...
                id_responses_emotion = inputs2['responses_emotion']
                ...
                x = self.atten_post_To_state(
                    torch.cat((state_posts[-1, :, :], atten_weight_post), dim=-1))  # [batch, dim]
                ...
                z2 = _mu + (0.5 * _logvar).exp() * sampled_latents  # [batch, latent]

                z_ = z2.detach().cpu().numpy()
                z_s.append(z_)
                label_s_act.append(id_responses_act.detach().cpu().numpy())
                label_s_emotion.append(id_responses_emotion.detach().cpu().numpy())
        z_s = np.concatenate(z_s, 0)
        # label_s_act = np.concatenate(label_s_act, 0)
        label_s_emotion = np.concatenate(label_s_emotion, 0)
        return z_s, label_s_emotion
