...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.u = torch.randn((act_num, config.latent_size), out=None).cuda()
        # 输出层
    ...
    def forward(self, inputs, word2vec, inference=False, ms = False ,inpre=False, ingau=False, max_len=60, gpu=True):
        if not inference:  # 训练
            ...
            u_ = self.u.unsqueeze(dim=0).repeat(z.size(0),1,1)#u_ [batch, num_cat, latent]
            z_ = z.unsqueeze(dim=0).repeat(act_num,1,1).transpose(0,1)#z_[batch, num_cat, latent]
            q = (1 + torch.pow((u_-z_),2))/torch.sum(1 + torch.pow((u_-z_),2), dim=1).unsqueeze(dim=1).repeat(1,act_num,1)#q [batch, num_cat, latent]
            f = torch.sum(q,dim=1).unsqueeze(dim=1).repeat(1,act_num,1) # [batch,num_cat,latent]
            p = (torch.pow(q,2)/f)/torch.sum(torch.pow(q,2)/f,dim=1).unsqueeze(dim=1).repeat(1,act_num,1)#[batch,num_cat,latent]
            classify_result = self.dec_softmax(torch.sum(q,dim=2)) # [batch, cat_num]
            input_cat = torch.bmm(embed_catgory.transpose(1, 2), torch.unsqueeze(classify_result, dim=2)).transpose(1,
                                                                                                                    2).transpose(
                0, 1)  # [1, batch, embed_size]
            # 解码器的输入为回复去掉end_id
            ...
            return output_vocab, _mu, _logvar, mu, logvar, p,q
        else:  # 测试
            id_posts = inputs['posts']  # [batch, seq]
            len_posts = inputs['len_posts']  # [batch]
            sampled_latents = inputs['sampled_latents']  # [batch, latent_size]
            ...
            u_ = self.u.unsqueeze(dim=0).repeat(z.size(0), 1, 1)  # u_ [batch, num_cat, latent]
            z_ = ms_z.unsqueeze(dim=0).repeat(act_num, 1, 1).transpose(0, 1)  # z_[batch, num_cat, latent]
            q = (1 + torch.pow((u_ - z_), 2)) / torch.sum(1 + torch.pow((u_ - z_), 2), dim=1).unsqueeze(dim=1).repeat(1,
                                                                                                                      act_num,
                                                                                                                      1)  # q [batch, num_cat, latent]
            f = torch.sum(q, dim=1).unsqueeze(dim=1).repeat(1, act_num, 1)  # [batch,num_cat,latent]
            p = (torch.pow(q, 2) / f) / torch.sum(torch.pow(q, 2) / f, dim=1).unsqueeze(dim=1).repeat(1, act_num,
                                                                                                      1)  # [batch,num_cat,latent]
            classify_result = self.dec_softmax(torch.sum(q, dim=2))  # [batch, cat_num]
    ...
    def save_model(self, epoch, global_step, path):
        ...
        torch.save({'embedding': self.embedding.state_dict(),
                    ...
                    'dec_softmax': self.dec_softmax.state_dict(),
                    'epoch': epoch,
    ...
    def load_model(self, path):
        ...
        self.dec_softmax.load_state_dict(checkpoint['dec_softmax'])
