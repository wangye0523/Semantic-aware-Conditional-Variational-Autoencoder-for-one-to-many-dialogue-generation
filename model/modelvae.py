...
class Model(nn.Module):
    ...
    def forward(self, inputs, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if not inference:  # 训练
            ...
            # p(z|x)
            _mu, _logvar =  self.recognize_net(x, y) # [batch, latent]
            # p(z|x,y)
            mu, logvar = self.prior_net(x)  # [batch, latent]
            # 重参数化
            ...
            return KLD, output_vocab, _mu, _logvar, mu, logvar, Loss, yita_c,pi, log_sigma2_c, mu_c
        else:  # 测试
            id_posts = inputs['posts']  # [batch, seq]
            len_posts = inputs['len_posts']  # [batch]
            sampled_latents = inputs['sampled_latents']  # [batch, latent_size]
            labels_id = inputs['labels_id']
