...
class Model(nn.Module):
    def __init__(self, config):
        ...
        # 分类器
        self.classifier = Decoder(config.decoder_cell_type,  # rnn类型
                               config.embedding_size,  # 输入维度
                               config.decoder_output_size,  # 输出维度
                               config.decoder_num_layers,  # rnn层数
                               config.dropout)  # dropout概率
    ...
    def forward(self, inputs, word2vec, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if not inference:  # 训练
            ...
            outputs_classify = []
            output_classify, _ = self.classifier(decoder_inputs[0], first_state)
            outputs_classify.append(output_classify)
            outputs_classify = torch.cat(outputs_classify, 0).transpose(0, 1)  # [batch, seq-1, dim_out]
            output_classify_vocab = self.projector(outputs_classify)  # [batch, seq-1, num_vocab]



            return output_vocab, _mu, _logvar, mu, logvar, Loss, output_classify_vocab
        else:  # 测试
            id_posts = inputs['posts']  # [batch, seq]
            len_posts = inputs['len_posts']  # [batch]
            sampled_latents = inputs['sampled_latents']  # [batch, latent_size]
            ...
            outputs_classify = []
            output_classify, _ = self.classifier(word2vec.embedding(first_input_id), first_state)
            outputs_classify.append(output_classify)
            outputs_classify = torch.cat(outputs_classify, 0).transpose(0, 1)  # [batch, seq-1, dim_out]
            output_classify_vocab = self.projector(outputs_classify)  # [batch, seq-1, num_vocab]

            return output_vocab, _mu, _logvar, None, None, output_classify_vocab
    ...
    def save_model(self, epoch, global_step, path):
        ...
        torch.save({'embedding': self.embedding.state_dict(),
                    ...
                    'mlp_classify': self.mlp_classify.state_dict(),
                    'pi_': self.pi_,
                    'mu_c': self.mu_c,
                    'log_sigma2_c': self.log_sigma2_c,
                    'mu1': self.mu1,
    ...
    def load_model(self, path):
        ...
        self.mlp_classify.load_state_dict(checkpoint['mlp_classify'])
