...
class Model(nn.Module):
    ...
    def forward(self, inputs,word2vec,gaussian_model, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if inpre:
            ...
            # p(z|x,y)
            mu, logvar = self.recognize_net(x, y)  # [batch, latent]
            np.save('miu_test', _mu.detach().cpu().numpy())
            np.save('miu_train', mu.detach().cpu().numpy())
            np.save('sigma_test', _logvar.detach().cpu().numpy())
            np.save('sigma_train', logvar.detach().cpu().numpy())
            input()
            # print(gaussian_kld(_mu, _logvar, mu, logvar))
            self.mu1 = mu
            self.sigma1 = logvar
            # 重参数化
            z = mu + (0.5 * logvar).exp() * sampled_latents  # [batch, latent]
            ...
            len_posts = inputs['len_posts']  # [batch]
            sampled_latents = inputs['sampled_latents']  # [batch, latent_size]
            str_responses_act = inputs['str_responses_act']
            ...
            y = self.atten_post_To_state(
                torch.cat((state_responses[-1, :, :], atten_weight_response), dim=-1))  # [batch, dim]
            # p(z|x)
            _mu, _logvar = gaussian_model[int(str_responses_act[0][0])-1].recognize_net(x, y)  # [batch, latent]
