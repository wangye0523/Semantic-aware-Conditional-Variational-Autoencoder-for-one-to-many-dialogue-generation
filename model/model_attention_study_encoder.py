...
class Model(nn.Module):
    ...
    def forward(self, inputs,word2vec, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if inpre:
            ...
            y = self.atten_post_To_state(torch.cat((state_responses[-1, :, :], atten_weight_response),dim=-1))  # [batch, dim]
            # p(z|x)
            _mu, _logvar = self.prior_net(x)  # [batch, latent]
            # p(z|x,y)
            mu, logvar = self.recognize_net(x, y)  # [batch, latent]
            ...
            if isinstance(state_posts, tuple):
                atten_weight_post = state_posts[2]
                state_posts = state_posts[0]
            x = self.atten_post_To_state(torch.cat((state_posts[-1, :, :], atten_weight_post), dim=-1))
            ...
            return output_vocab, _mu, _logvar, None, None,torch.stack(attentions,dim=0).transpose(0, 1)
