...
from model.Encoder_attention_all import Encoder
...
class Model(nn.Module):
    def __init__(self, config):
        ...
        self.atten_post_To_state = nn.Linear(config.decoder_output_size * 2, config.decoder_output_size)
        self.atten_response_To_state = nn.Linear(config.decoder_output_size * 2, config.decoder_output_size)
    ...
    def forward(self, inputs,word2vec, inference=False, inpre=False, ingau=False, max_len=60, gpu=True):
        if inpre:
            ...
            if isinstance(state_posts, tuple):
                atten_weight_post = state_posts[2]
                state_posts = state_posts[0]
            if isinstance(state_responses, tuple):
                atten_weight_response = state_responses[2]
                state_responses = state_responses[0]
            x = self.atten_post_To_state(torch.cat((state_posts[-1, :, :], atten_weight_post), dim=-1))  # [batch, dim]
            y = self.atten_post_To_state(
                torch.cat((state_responses[-1, :, :], atten_weight_response), dim=-1))  # [batch, dim]
            ...
            classify_result = self.classification(z)
            ...
            for idx in range(len_decoder):
                if idx == 0:
                    if ingau == 0:
                        state = self.no_emotion_prepare_state(torch.cat([z, x], 1))  # 解码器初始状态
                    elif ingau == 1:
                        state = self.positive_prepare_state(torch.cat([z, x], 1))  # 解码器初始状态
                    else:
                        state = self.negative_prepare_state(torch.cat([z, x], 1))  # 解码器初始状态
                decoder_input = decoder_inputs[idx]  # 当前时间步输入 [1, batch, embed_size]
                # output: [1, batch, dim_out]
                # state: [num_layer, batch, dim_out]
                if ingau == 0:
                    output, state = self.no_emotion_decoder(decoder_input, state, encoder_output)
                elif ingau == 1:
                    output, state = self.positive_decoder(decoder_input, state, encoder_output)
                else:
                    output, state = self.negative_decoder(decoder_input, state, encoder_output)
                outputs.append(output)

            outputs = torch.cat(outputs, 0).transpose(0, 1)  # [batch, seq-1, dim_out]
            if ingau == 0:
                output_vocab = self.projector_no_emotion(outputs)  # [batch, seq-1, num_vocab]
            elif ingau == 1:
                output_vocab = self.projector_positive(outputs)  # [batch, seq-1, num_vocab]
            else:
                output_vocab = self.projector_negative(outputs)  # [batch, seq-1, num_vocab]
            ...
            classify_result = self.classification(z)# [batch, num_catgory]
            ...
            outputs_0, outputs_1, outputs_2, outputs = [], [], [], []
            for num_catgory in range(0, emotion_num):
                if num_catgory == 0:
                    ...
                    for idx in range(max_len):
                        ...
                        vocab_prob = self.projector_no_emotion(output)  # [1, batch, num_vocab]
                    ...
                    outputs_0 = self.projector_no_emotion(outputs_0)  # [batch, seq, num_vocab]
                elif num_catgory == 1:
                    done = torch.tensor([0] * batch_size).bool().cuda()
                    for idx in range(max_len):
                        if idx == 0:  # 第一个时间步
                        ...
                        vocab_prob = self.projector_positive(output)  # [1, batch, num_vocab]
                        next_input_id = torch.argmax(vocab_prob, 2)  # 选择概率最大的词作为下个时间步的输入 [1, batch]
                    ...
                    outputs_1 = self.projector_positive(outputs_1)  # [batch, seq, num_vocab]
                else:
                    done = torch.tensor([0] * batch_size).bool().cuda()
                    for idx in range(max_len):
                        if idx == 0:  # 第一个时间步
                        ...
                        vocab_prob = self.projector_negative(output)  # [1, batch, num_vocab]
                        next_input_id = torch.argmax(vocab_prob, 2)  # 选择概率最大的词作为下个时间步的输入 [1, batch]
                    ...
                    outputs_2 = self.projector_negative(outputs_2)  # [batch, seq, num_vocab]
            done = torch.tensor([0] * batch_size).bool().cuda()
            for idx in range(max_len):
                if idx == 0:  # 第一个时间步
                    state = first_state  # 解码器初始状态
            ...
            return outputs_0, outputs_1, outputs_2,outputs, clf_result
    ...
    def save_model(self, epoch, global_step, path):
        ...
        torch.save({'embedding': self.embedding.state_dict(),
                    ...
                    'atten_post_To_state': self.atten_post_To_state.state_dict(),
                    'atten_response_To_state': self.atten_response_To_state.state_dict(),
                    'epoch': epoch,
    ...
    def load_model(self, path):
        ...
        self.atten_post_To_state.load_state_dict(checkpoint['atten_post_To_state'])
        self.atten_response_To_state.load_state_dict(checkpoint['atten_response_To_state'])
